...
3. **`ui/`** -- Display rendering, no I/O.
   ...
   - `radar_range` -- All persisted view state, not just range: preset index (5/10/15/25 km), miles/km, cumulative airport tier, heading rotation in 15 degree steps, label mode, poll rate, sweep, military highlight, LAN portal flag, tags flag. Persists to NVS (`planeradar` namespace). Adding a setting means a key here and an entry in `menu.cpp`.
